...
public class CameraFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (focusArea.moveDistance.x != 0)
        {
            lookAheadDirectionX = Mathf.Sign(focusArea.moveDistance.x);
            if (Mathf.Sign(target.playerInput.x) == Mathf.Sign(focusArea.moveDistance.x) && target.playerInput.x != 0)
            {
        ...
        transform.position = (Vector3)focusPosition + Vector3.forward * -50;
    }
    ...
    struct FocusArea
    {
        ...
        public void Update(Bounds targetBounds)
        {
            ...
            center = new Vector2((left + right) / 2, (top + bottom) / 2);
            moveDistance = new Vector2(shiftX, shiftY);
        }
    }
}
